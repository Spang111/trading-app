...
async function readResponseMessage(response: Response) {
  try {
    ...
    return translateBackendMessage(data.detail || data.message || `Request failed with status ${response.status}`)
  } catch {
...
export function AuthDialog({
  ...
  const title = mode === "login" ? "登录 AI Trading" : "注册 AI Trading 账号"
  const description =
    mode === "login"
      ? "登录后可以查看订阅、策略和个人控制台。"
  ...
  async function handleLoginSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : "暂时无法登录，请稍后再试。"
  ...
  async function handleRegisterSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFormError(
        error instanceof Error ? error.message : "暂时无法注册，请稍后再试。",
      )
    } finally {
      setIsSubmitting(false)
    }
